...
const web3 = new Web3(); // 替换为你的以太坊节点地址
...
async function testMain() {
	try {
		...
		console.log("代币总供应量:", web3.utils.fromWei(Number(totalSupply), "ether"));

		// 查询地址余额
		const balance: string = await contract.methods.balanceOf(address).call();
		console.log("地址余额:", web3.utils.fromWei(Number(balance), "ether"));
